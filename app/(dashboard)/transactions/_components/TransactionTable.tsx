...
const columns : ColumnDef<TransactionHistoryRow>[] = [
  {
    ...
    cell: ({row}) => (
      <div className="flex items-center gap-2 capitalize">
        {row.original.categoryIcon} <div className="capitalize">{row.original.category}</div> 
      </div>
    )
  },
  {
  ...
  },
  {
    ...
    cell: ({row}) => (
      <p className="text-md rounded-lg bg-gray-400/5 text-center font-medium">{row.original.amount}</p>
    )
  },
  {
    id: "actions",
  ...
  const categoriesOptions = useMemo(() => {
    ...
    history.data?.forEach((row) => {
      categoriesMap.set(row.category, {
        value: row.category,
        label: `${row.categoryIcon} ${row.category}`,
      });
    });
